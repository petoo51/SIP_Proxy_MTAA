        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                if call_id not in active_calls:
                    ...
                    diary_file = open("call_log.txt", "a")
                    diary_file.write("TIME: " + time_str + " CALL ID: " + call_id + " CALLING " + destination + " FROM " + origin + "\n")
                    diary_file.close()
                logging.info("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
            else:
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                for line in data:
                    if rx_callid.search(line):
                        call_id = line[9:]
                if call_id not in active_calls_ack:
                    active_calls_ack.append(call_id)
                    time_str = time.strftime("[%H:%M:%S]", time.localtime())
                    diary_file = open("call_log.txt", "a")
                    diary_file.write("TIME: " + time_str + " CALL ID: " + call_id + " CALL BETWEEN " + destination + " AND " + origin + " STARTED" + "\n")
                    diary_file.close()
                logging.info("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                bye = False
                for line in data:
                    if rx_bye.search(line):
                        bye = True
                    elif rx_callid.search(line):
                        call_id = line[9:]
                if bye:
                    if call_id in active_calls:
                        active_calls.remove(call_id)
                        active_calls_ack.remove(call_id)
                    time_str = time.strftime("[%H:%M:%S]", time.localtime())
                    diary_file = open("call_log.txt", "a")
                    diary_file.write("TIME: " + time_str + " CALL ID: " + call_id + " CALL BETWEEN " + destination + " AND " + origin + " ENDED" + "\n")
                    diary_file.close()
                # insert Record-Route
                data.insert(1, recordroute)
                text = "\r\n".join(data)
